...
- Fixed multiple typos and incorrect error-format verbs (`%e`) in user-facing
  log and error messages across `migfaker`, `kubeclient`, `apprunner`,
  `dra-plugin-gpu`, `status-updater`, and `kwok-gpu-device-plugin`.
