...
// It also handles late allocation - when the Add event happened before the ResourceClaim was allocated.
func (p *PodHandler) handleDraGpuPodUpdate(pod *v1.Pod, nodeTopology *topology.NodeTopology) error {
	if !util.IsDraPod(pod) {
		return nil
	}

	// First, check if the GPU is already allocated to this pod
	isAllocated := isDraAlreadyAllocated(pod, nodeTopology)

	if !isAllocated {
		// GPU not yet allocated - try to allocate now (late allocation)
		// This handles the case where Add event happened before ResourceClaim was allocated
		deviceNames, err := p.getDeviceNamesFromClaims(pod)
		if err != nil {
			return fmt.Errorf("failed to get device names from claims for pod %s: %w", pod.Name, err)
		}
		...
		if len(deviceNames) > 0 {
			log.Printf("DRA Update: Late allocation for pod %s with %d devices: %v\n", pod.Name, len(deviceNames), deviceNames)
			for _, deviceName := range deviceNames {
				gpuIdx := findGpuIndexByID(nodeTopology, deviceName)
				if gpuIdx == -1 {
					log.Printf("GPU device %s not found in node topology for DRA pod %s\n", deviceName, pod.Name)
				...
				gpu := &nodeTopology.Gpus[gpuIdx]
				if gpu.Status.AllocatedBy.Pod == "" {
					log.Printf("DRA Update: GPU %s is free, allocating to pod %s...\n", gpu.ID, pod.Name)
					gpu.Status.AllocatedBy.Namespace = pod.Namespace
					gpu.Status.AllocatedBy.Pod = pod.Name
					gpu.Status.AllocatedBy.Container = getContainerWithClaim(pod)
		...
		// GPU already allocated - just update usage status
