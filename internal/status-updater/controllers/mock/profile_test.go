...
func TestRenderConfig_NoOverrides(t *testing.T) {
	...
	out, _, err := RenderConfig(kube, "ns", topology.GpuConfig{Profile: "a100"})
...
func TestRenderConfig_ScalarOverride(t *testing.T) {
	...
	out, _, err := RenderConfig(kube, "ns", topology.GpuConfig{
		Profile: "a100",
...
func TestRenderConfig_MissingProfileCM(t *testing.T) {
	...
	_, _, err := RenderConfig(kube, "ns", topology.GpuConfig{Profile: "h100"})
	require.Error(t, err)
...
func TestRenderConfig_MalformedProfileYAML(t *testing.T) {
	...
	kube := fake.NewSimpleClientset(cm)
	_, _, err := RenderConfig(kube, "ns", topology.GpuConfig{Profile: "broken"})
	require.Error(t, err)
}

func TestRenderConfig_EmptyProfile(t *testing.T) {
	out, _, err := RenderConfig(fake.NewSimpleClientset(), "ns", topology.GpuConfig{})
	assert.Error(t, err, "empty profile name must error")
	assert.Nil(t, out)
}

func TestRenderConfig_ExtractsGpuSpec(t *testing.T) {
	cm := newProfileCM("gpu-profile-a100", "ns", a100ProfileYAML)
	kube := fake.NewSimpleClientset(cm)

	_, spec, err := RenderConfig(kube, "ns", topology.GpuConfig{Profile: "a100"})
	require.NoError(t, err)
	assert.Equal(t, "550.163.01", spec.DriverVersion, "driver version extracted from system.driver_version")
	assert.Equal(t, 1, spec.GpuCount, "GPU count derived from len(devices)")
}
